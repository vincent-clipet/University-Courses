...
while (true) do
  ...
  if (todo == "a")
    $toile.setAngle($toile.getAngle() - 0.1);
    session.puts($toile.getAngle());
  elsif (todo == "A")
    $toile.setAngle($toile.getAngle() + 0.1);

  elsif (todo == "x")
    $toile.setMidX($toile.getMidX() - 2);
  elsif (todo == "X")
    $toile.setMidX($toile.getMidX() + 2);

  elsif (todo == "y")
    $toile.setMidY($toile.getMidY() - 2);
    session.puts($toile.getMidY());
  elsif (todo == "Y")
    $toile.setMidY($toile.getMidY() + 2);

  elsif (todo == "r")
    $toile.setR($toile.getR() - 2);
  elsif (todo == "R")
    $toile.setR($toile.getR() + 2);
